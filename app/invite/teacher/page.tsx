...
function TeacherInvitePageContent() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { userId: teacherId, needsConfirmation } = await registerUser({
        email: invite.email,
        password,
        displayName,
        role: "teacher",
      });
      await consumeTeacherInvite(invite.token, teacherId);
      if (needsConfirmation) {
        pushToast(
          `Account created. Confirm the link we sent to ${invite.email}, then log in.`,
          "success",
        );
        router.push("/login");
        return;
