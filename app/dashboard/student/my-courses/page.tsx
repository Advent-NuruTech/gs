...
export default function MyCoursesPage() {
  ...
  const titlesById = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(courseLookup).map(([id, course]) => [id, course?.title]),
      ),
    [courseLookup],
  );
    ...
    <section className="space-y-4">
      ...
      <PaymentDrafts
        ...
        onCancelled={(reference) =>
          setPayments((current) =>
            current.filter((payment) => payment.paystackReference !== reference),
          )
        }
      />

      {activeEnrollments.length === 0 && completedEnrollments.length === 0 && pendingDrafts.length === 0 ? (
